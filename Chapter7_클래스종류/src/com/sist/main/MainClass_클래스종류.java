 ...
 *         = 캡슐화
 *            데이터보호와 동시에 필요시에 사용이 가능하게 만든다 ( 변수)
 *              => 변수 (읽기/쓰기) ==> getter/setter
 *      
 *         = 상속 / 포함 : 재사용
 *              상속 : 변경이 필요한 경우 => 게시판 => 갤러리 게시판
 *              포함 : 변경없이 사용     => 게시판 => 댓글
 *              상속 => 예외(초기화블록, 생성자, static, private형식)
 *                  class A extends B
 *                       ---       ---
 *                     하위클래스   상위클래스
 *                   B의 모든 내용을 상속받아서 A라는 새로운 클래스를 만든다
 *                   => 단일 상속만 가능하다
 *                  class A
 *                  {
 ...
 *                  상위 클래스 : this
 *                  하위 클래스 : this, super
 *         super() => 상속 내린 클래스의 생성자 호출 (생성자 안에서 사용 => 첫줄)
 *         
 *         예)
 *          public class Parent
 *          {
 *            public Parent(int a){}
 *            //default 생성자는 존재하지 않음
 *          }
 ...
 *      그 외 : static, final, abstraact, synchronized
 *      static : 공통적인
 *      ----------------> 클래스, 메소드, 변수 => 지역변수에서는 사용할 수 없다
 *      final : 마지막
 *      ----------------> 클래스, 메소드, 변수, 지역변수
 *             public final class A => 확장할 수 없는 클래스(종단 클래스) => 상속을 내리지 못함
 ...
 *      지역변수 final
 *      -----------------------------------
 *      * 메소드에 static / abstract 는 동시에 사용이 불가능하다
 *        접근지정어 제어자 
 ...
 *      * 메소드중에 private, final 을 동시에 사용 할 수 없다
 *      
 *      추상클래스 : 자신이 메모리 할당을 할 수 없다, 미완성된 클래스 => 사용시는 구현된 클래스의 주소를 받아서 사용
 *       class A
 *       class B extends A
 *       
 *       B b=new B(); ==> 불가능
...
abstract class 도형{
	...
	//abstract가 붙으면 모든 클래스에서 공통으로 구현해야 한다.
	
	//선택적으로 구현할 수 있게 하려면 abstract를 뺀다 (오버라이딩)
