 ...
 *  ============================ + 클래스(객체지향)
 *  
 *  1. 메소드 : 특정작업을 수행하는 명령문을 모아서 관리
 *  			---------- 한가지 기능 (로그인...목록.... etc)
 *  			데이터 저장 : 변수, 배열
 *  			명령문 (cpu) : 메소드 ==> 명령
 *  			다른 클래스와 연결
 *  			메소드는 같은 이름으로 만들 수 있다 (데이터 타입이나 매개변수가 다르면 된다)			
 *  
 *  return : 메소드 종료를 알림 ( 꼭 마지막에만 올 수 있는것은 아니다)
 *  		
 *  
 *   결과값	매개변수(지역변수의 일종) => 메소드종료와 동시에 메모리에서 사라지는 변수 (Stack)메모리에서 관리
 * ------------------
 * 	o         o		==>  로그인 : 사용자 입력(id,pwd) =>결과값 boolean
 * 						 printf ==> 가변 매개변수
 * 						 String...arg => String형
