...
public class MusicManager {
	...
	static {
		try {
			...
			String mu="";
			int i=0; //read() => 문자 1개를 읽음 => i 문자번호 ==> (char)i
			while((i=fr.read())!=-1) { // -1 파일 끝 (EOF = End Of File)
			...
			for(String s:m) {
				...
				music[i].setKey(ss[4]);
				//변수에 직접 접근이 불가능 하기때문에 메소드를 이용해서 저장
				i++;
			}
		}catch(Exception e) {}
	}
    //초기화 블록 / 생성자 ==> 파일 읽기(구현) => 클래스영역에서는 할 수 없기에 생성자 or 초기화블럭 이용
	/*
	 *   변수의 초기화
	 *   인스턴스
	 *   정적변수(static)
	 ...
	 *     인스턴스 블록 : 인스턴스 변수 초기화
	 *     {
	 *      인스턴스변수, static 변수 초기화 가능
	 *     }
	 *     
	 *     static : static 초기화
	 *     {
	 *       static 변수만 초기화( 인스턴스변수 사용불가)
	 *     }
	 *     
